...
import { addToPackageCartThunk, deletePackageCartThunk, removeFromPackageCartThunk } from "../../redux/packageCartSlice";

const PackageCartItems = (props) => {
    ...
    const handleRemoveFromCart = () => {
        const productId = props.productId;
        setLoading2(true);
        dispatch(removeFromPackageCartThunk({ productId }))
            .then((res) => {
                if (res.payload.data.success) {
                    setLoading2(false);
    ...
    const deleteProduct = () => {
        setLoading3(true);
        const productId = props.productId;
        dispatch(deletePackageCartThunk({ productId })).then((res) => {
            if (res.payload.data.success) {
                setLoading3(false);
...
export default PackageCartItems;
